...
export async function parsePSDFromURL(url) {
    return await PSD.fromURL(url);
}
function toRGBAColor(data) {
    const [r, g, b] = data;
    let [, , , a] = data;
    if (a > 1) {
        a = a / 255;
...
function toTransformMatrix({ xx, xy, yx, yy, tx, ty }) {
    return { a: xx, b: xy, c: yx, d: yy, tx, ty };
}
function toCloudTextConfig(data, layer) {
    ...
    let point = 0;
    const texts = StyleRun.RunArray.map((text, index) => {
...
function toCloudImageConfig(data, layer) {
    let src = [];
    try {
...
export async function convertPSD2Page(psd) {
    ...
    const findLayer = (data) => {
        const result = !data.visible ? null : psd.layers.find((layer) => layer.name === data.name && layer.top === data.top && layer.right === data.right && layer.bottom === data.bottom && layer.left === data.left && layer.width === data.width && layer.height === data.height);
        return result;
    };
...
export async function processPSD2Page(file) {
