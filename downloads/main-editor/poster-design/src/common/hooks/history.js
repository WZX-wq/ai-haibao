...
const blackClass = ['operation-item', 'icon-undo', 'icon-redo'];
const whiteKey = ['ArrowLeft', 'ArrowDown', 'ArrowRight', 'ArrowUp', 'Backspace', 'Delete', 'v'];
...
function noPutHistory(target) {
    const classList = Array.from(target.classList);
    return classList.filter((v) => blackClass.includes(v)).length > 0;
}
export default () => {
    ...
    //   processing = false
    // })
    diffLayouts.onmessage((changes) => {
        changes.patches.length > 0 && historyStore.changeHistory(changes);
        processing = false;
    });
    onMounted(() => {
        document.addEventListener('mousedown', (e) => {
            if (noPutHistory(e.target))
                return;
