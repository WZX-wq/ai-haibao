...
export default async function (type, item, data) {
    let setting = data;
    if (type === 'text') {
        !item.fontFamily && !item.color ? (setting = JSON.parse(JSON.stringify(wTextSetting))) : (setting = item);
        !setting.text ? (setting.text = '双击编辑文字') : (setting.text = decodeTextIfNeeded(setting.text)); // item.text
...
//# sourceMappingURL=setWidgetData.js.map