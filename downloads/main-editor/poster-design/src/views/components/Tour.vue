...
<template>
  <el-tour v-model="isShow">
    ...
    <el-tour-step
      v-if="hasRightPanelTarget"
      placement="left"
      :target="steps[2]?.$el"
      title="右侧属性栏"
      description="当选中画布中的元素时，在此处会显示相应的编辑界面；同时也可以切换到“图层”管理。"
    />
    <el-tour-step :target="downloadTarget" title="下载作品" description="点击此处即可导出当前作品，赶紧试试吧。" />
  </el-tour>
</template>
...
const hasRightPanelTarget = computed(() => Boolean(props.steps?.[2]?.$el))
const downloadTarget = computed(() => {
  if (hasRightPanelTarget.value) return props.steps?.[3]?.$el
  return props.steps?.[2]?.$el
})
