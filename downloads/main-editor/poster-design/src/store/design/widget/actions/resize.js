...
/** 设置 resize 操作的初始值 */
export function initDResize(store, payload) {
...
/** 更新组件宽高 */
export function dResize(store, { x, y, dirs }) {
...
export function resize(store, size) {
...
/** 自适应适配所有元素 */
export function autoResizeAll(store, lastPageSize) {
    if (!lastPageSize)
        return;
    const canvasStore = useCanvasStore();
