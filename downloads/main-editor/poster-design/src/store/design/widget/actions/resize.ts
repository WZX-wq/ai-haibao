  ...
  for (let i = 0; i < dirs.length; ++i) {
    ...
    switch (dir) {
      case 'top':
        ...
        top = Math.max(t, 0)
        top = Math.min(widgetXY.y + resizeWH.height - (target.record?.minHeight || 0), top)
        target.height += target.top - top
        target.height = Math.max(target.height, target.record?.minHeight || 0)
        target.top = top
        break
      case 'bottom':
      ...
      case 'left':
        ...
        target.width = Math.max(target.width, target.record?.minWidth || 0)
        left = Math.min(widgetXY.x + resizeWH.width - (target.record?.minWidth || 0), left)
        target.left = left
        break
      case 'right':
