...
/** 设置 mousemove 操作的初始值 */
export function initDMove(store, payload) {
...
/** 移动组件 */
export function dMove(store, payload) {
...
export function updateGroupSize(store, uuid) {
...
export function updateHoverUuid(store, uuid) {
    store.dHoverUuid = uuid;
}
/**
 * 设置拖拽时在哪个图层
 */
export function setDropOver(store, uuid) {
    store.dDropOverUuid = uuid;
}
export function setMouseEvent(state, e) {
    state.activeMouseEvent = e;
}
export function setdActiveElement(state, data) {
