      ...
      <div class="card-header">
        ...
        <el-popover :visible="state.visiable" placement="bottom-end" :width="260" trigger="click">
          ...
          <div class="select__box">
            ...
            >
              <el-image :src="item.thumb" fit="contain" />
            </div>
          </div>
          <template #reference>
...
const select = (value: string = '') => {
  ...
  if (dActiveElement.value?.uuid && dActiveElement.value.type === 'w-image') {
    ;(dActiveElement.value as Record<string, any>).mask = value
    ...
    nextTick(() => {
      const imageBox = document.getElementById(dActiveElement.value?.uuid || '')?.querySelector('.img__box') as HTMLElement | null
      if (imageBox) {
        const maskValue = getMaskStyleValue(value)
...
const getMaskStyleValue = (value = '') => {
  if (!value) {
    return 'initial'
  }
  return `url(${value.replace(/'/g, '%27').replace(/\s+/g, '%20')})`
}
