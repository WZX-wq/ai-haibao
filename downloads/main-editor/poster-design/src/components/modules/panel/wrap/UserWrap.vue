  ...
  api.home
    ...
    .then(({ list = [] }) => {
      ...
      const normalizedList = list.map((x) => {
        ...
        return {
          ...x,
          thumb: appendParams(preferredCover),
          cover: appendParams(coverUrl),
          url: appendParams(previewUrl),
        }
      })
...
const deleteWorks = async ({ i, item }: controlImgParam) => {
  ...
  try {
    ...
    eventBus.emit('refreshUserDesigns', { id: item.id })
    const currentId = String(route.query.tempid ?? route.query.id ?? '')
    if (currentId && currentId === String(item.id)) {
      router.replace({
        path: '/home',
        query: {
          section: 'mine',
...
eventBus.on('refreshUserDesigns', () => {
  state.designList = []
  state.isDone = false
  listPage = 0
