...
-->
<template>
  <div ref="root_ref" class="db-root" :class="{ 'db-root--embed': embedPreview }" :id="embedRootId">
    <div
      ref="page_design"
      class="db-scroll"
      :id="embedScrollId"
      :style="{
        paddingTop: (embedPreview ? 0 : dPaddingTop) + 'px',
        minWidth: (dPage.width * dZoom) / 100 + (padding ?? dPresetPadding) * 2 + 'px',
      }"
    >
      <div
...
type TProps = {
  ...
  zoom?: number
  /** 嵌入多实例（如欢迎页缩略预览）：唯一 id、关闭缩放工具、避免与主编辑器的 #main 冲突 */
...
const props = withDefaults(defineProps<TProps>(), { embedPreview: false })

const root_ref = ref<HTMLElement | null>(null)
const page_design = ref<HTMLElement | null>(null)
const embedKey = computed(() =>
  String(props.pageDesignCanvasId || 'canvas')
    .replace(/[^a-zA-Z0-9_-]/g, '-')
    .slice(0, 96),
)
const embedRootId = computed(() => (props.embedPreview ? `db-main-${embedKey.value}` : 'main'))
const embedScrollId = computed(() => (props.embedPreview ? `db-scroll-${embedKey.value}` : 'page-design'))

const needTools = computed(
  () => !props.embedPreview && route.name !== 'Draw' && route.name !== 'Html' && route.name !== 'AiPoster',
)
...
onMounted(() => {
  getScreen()
  const pageDesignEl = page_design.value
  ...
  // 绑定空格事件
  const scrollContainer: any = props.embedPreview ? root_ref.value : document.querySelector('#main')
...
async function drop(e: MouseEvent) {
  ...
  // 绝对坐标
  const canvasEl = document.getElementById(props.pageDesignCanvasId)
  if (!canvasEl) return
...
.db-root {
  ...
  background:
    ...
    linear-gradient(180deg, #f7f2e9 0%, #edf3fb 100%);
}
.db-scroll {
  ...
  padding-inline: 28px;
  background-image:
    linear-gradient(rgba(26, 43, 72, 0.06) 1px, transparent 1px),
    linear-gradient(90deg, rgba(26, 43, 72, 0.06) 1px, transparent 1px);
  ...
  .out-page {
    ...
    box-shadow:
      ...
      0 24px 60px rgba(29, 47, 76, 0.12);
    .design-canvas {
      // 解决拖拽残影问题
      ...
      border-radius: 24px;
      box-shadow:
        0 24px 50px rgba(20, 34, 60, 0.18),
        0 4px 14px rgba(20, 34, 60, 0.08);
      margin: 0 auto;
      position: relative;
      overflow: hidden;
    }
...
.db-root--embed .out-page {
  border-radius: 12px;
  box-shadow: none;
  background: transparent;
}
.db-root--embed .design-canvas {
  border-radius: 10px;
